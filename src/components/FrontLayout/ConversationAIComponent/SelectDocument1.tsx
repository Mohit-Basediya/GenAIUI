...
export default function SelectDocument1({
  updateSummary,
  callAPI,
  setLoading,
}: any) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("https://inteligenius.azurewebsites.net/filename_metadata");
        const data = await response.json();
        const filenamesWithoutExtension = data.map((item: ApiResponseItem) =>
          item.filename.replace(/\.[^/.]+$/, "")
        );
        // console.log(filenamesWithoutExtension);
        setFilenames(filenamesWithoutExtension)
      } catch (error) {
  ...
  const handleMetadata = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    setFilename(value);
    if (value === "") {
      setSuggestions([]);
